  ...
  */
  shouldIncludeChildAddon(childAddon) {
    ...
    if (this.project.seenAddons) {
      ...
      const shouldKeep = alwaysKeepAddons.some(v => childAddon.name.indexOf(v) >= 0);
      if (!shouldKeep && seenVersion && (seenVersion === 'symlink' || semver.gte(seenVersion, childAddon.pkg.version))) {
