...
EmberApp.prototype.addonTreesFor = function(type) {
  return this.project.addons.map(addon => {
    const seenVersion = this.project.seenAddons[addon.name];
    const addonVersion = addon.pkg.version
    if ((!seenVersion || (seenVersion === addonVersion)) && addon.treeFor) {
      console.log(`Including ${addon.name}'s treeFor ${type}, version ${addonVersion}`)
      if (type === 'addon' && addonVersion === '8.3.4' ) debugger
